...
describe('DamHolder Functionality', function () {
	describe('Functionality', function () {
		it('Should successfully authorize an operator', async function () {
			...
			expect(await damToken.isOperatorFor(operator, damHolder.target)).to.equal(true);
		});
		...
		it('Should fail to authorize operator for a non-ERC777 token', async function () {
			const { damHolder, addrB, ethers } = await loadFixture(deployDamHolderFixture);
			...
			await expect(damHolder.authorizeOperator(nonErc777TokenAddress, operator)).to.be.revertedWithoutReason(ethers);
		});
	});
});

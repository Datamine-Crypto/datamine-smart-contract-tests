/**
 *Submitted for verification at Arbiscan.io on 2024-10-13
*/
...
// File: contracts/LockquidityToken.sol
...
/**
 * @dev Datamine Crypto - Lockquidity Smart Contract
 */
contract LockquidityToken is ERC777, IERC777Recipient {
    /**
    ...
    /**
     * @dev This will be Vault for the LOCK token smart contract address
     */
    address immutable private _vaultAddress;

    IERC1820Registry private _erc1820 = IERC1820Registry(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24);
    bytes32 constant private TOKENS_RECIPIENT_INTERFACE_HASH = keccak256("ERC777TokensRecipient");
	
    /**
     * @dev Decline some incoming transactions (Only allow Lockquidity smart contract to send/recieve ArbiFLUX tokens)
     */
    function tokensReceived(
        address operator,
        address from,
        ...
        // Ensure someone doesn't send in some ArbiFLUX to this contract by mistake (Only the contract itself can send itself ArbiFLUX)
        require(operator == address(this) , "Only LOCK contract can send itself ArbiFLUX tokens");
        require(to == address(this), "Funds must be coming into LOCK token");
        require(from != to, "Why would LOCK contract send tokens to itself?");
    }
    ...
    constructor(address token, uint256 startTimeReward, uint256 maxTimeReward, uint256 failsafeBlockDuration, address vaultAddress) public ERC777("Lockquidity", "LOCK", new address[](0)) {
    ...
    /**
     * @dev 0.00000001 LOCK minted/block/1 ArbiFLUX
     * @dev 10^18 / 10^8 = 10^10
     */
    uint256 private constant _mintPerBlockDivisor = 10 ** 8;

    /**
     * @dev To avoid small LOCK/ArbiFLUX burn ratios we multiply the ratios by this number.
     */
	 ...
	 * @dev This is done because we're using integers without any decimals.
     */
    uint256 private constant _percentBurnMultiplier = 1;

    /**
     * @dev This is our max 10x LOCK burn multiplier. It's multiplicative with the time multiplier.
     */
	...
	/**
	 * @dev PUBLIC FACING: Burn LOCK tokens to a specific address
	 */
    ...
    public {
        require(amount > 0, "You must burn > 0 LOCK");
        ...
        // Call the normal ERC-777 burn (this will destroy LOCK tokens). We don't check address balance for amount because the internal burn does this check for us.
        _send(_msgSender(), address(_vaultAddress), amount, "", "", false);
    }

	/**
	 * @dev PUBLIC FACING: Mint LOCK tokens from a specific address to a specified address UP TO the target block
	 */
    ...
    public {
        ...
        // Call the normal ERC-777 mint (this will mint LOCK tokens to targetAddress)
    ...
    function getAddressBurnMultiplier(address targetAddress) public view returns(uint256) {
        ...
        uint256 burnMultiplier = Math.min(_maxBurnMultiplier, myRatio.mul(_percentMultiplier).div(globalRatio).add(_percentBurnMultiplier)); // Min 1x, Max 10x
        return burnMultiplier;
    }

    /**
     * @dev PUBLIC FACING: Get ArbiFLUX/LOCK burn ratio for a specific address
     */
    function getAddressRatio(address targetAddress) public view returns(uint256) {
        ...
        // Note that we multiply both ratios by the ratio multiplier before dividing. For tiny LOCK/ArbiFLUX burn ratios.
        uint256 myRatio = addressBurnedAmount.mul(_ratioMultiplier).div(addressLockedAmount);
        return myRatio;
    }

    /**
     * @dev PUBLIC FACING: Get ArbiFLUX/LOCK burn ratio for global (entire network)
     */
...
contract LockquidityVault {
    
    // New Lockquidity Uniswap addresses from https://docs.uniswap.org/contracts/v2/reference/smart-contracts/v2-deployments (Arbitrum section)
    address private constant FACTORY = 0xf1D7CC64Fb4452F05c498126312eBE29f30Fbcf9;
    ...
    function sweep() external {
        ...
        uint256 balA = IERC777(_token).balanceOf(address(this));

        // Swap the entire balance of this address and add as liquidity
        zap(address(_token), WETH, balA);
    }
...
contract LockquidityFactory {
    
    LockquidityToken public token;
    LockquidityVault public vault;
    address immutable public _lockableTokenAddress;

    constructor(address lockableTokenAddress) public {
        _lockableTokenAddress = lockableTokenAddress;

        vault = new LockquidityVault();

        token = new LockquidityToken(lockableTokenAddress, 5760, 161280,161280, address(vault));
